...
// seanse has the screeing info from database
                for (let i = 0; i < data.length; i++) {
                    ...
                    for (let z = 0; z < screening.length; z++) {
                        if (screening[z].movie_id == data[i].movie_id) { // check the right movie to put the link for the booking
                            let a = document.createElement("a");//create link element

                            a.innerHTML = "<span>" + screening[z].hour + "</span><span>" + screening[z].date + "</span>";//add content to link
                            a.href = "seats.php?id=" + screening[z].screening_id;//add the right id to link for the seats.js
