...
<div class="top-panel">
    ...
    <?php
        ...
        $sql = "SELECT reservation_id,row,seat,movie_id FROM `reservation`";
        $res = $conn->query($sql);
        $response = array();
...
echo '<table>';
echo '<tr><th>Reservation id</th><th>Row</th><th>Seat</th><th>Movie id</th>';
foreach ($response as $reservation) {
    ...
    echo '<td>' . $reservation['movie_id'] . '</td>';
    echo '</tr>';
}
echo '</table>';
